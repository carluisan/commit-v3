...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($id_usuario) || empty($nombre) || empty($apellido) || empty($domicilio) || empty($telefono) || empty($email)) {
        echo "Todos los campos son requeridos.";
    } else {
        $sql = "INSERT INTO usuario (id_usuario, nombre, apellido, domicilio, telefono, email) VALUES (?, ?, ?, ?, ?, ?)";
        
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ssssss", $id_usuario, $nombre, $apellido, $domicilio, $telefono, $email);
